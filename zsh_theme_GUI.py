...
def GUI(self, Gtk, main, zsh_themes, base_dir, GdkPixbuf):
    ...
    self.zsh_theme = Gtk.ComboBoxText()
    zsh_themes.get_themes(self.zsh_theme)
    hbox19.pack_start(self.zsh_theme, True, True, 10)
    ...
    if self.zsh_theme.get_active_text() is None:
        pass
    elif _functions.os.path.isfile(base_dir+"/images/zsh_previews/"+self.zsh_theme.get_active_text()+".jpg"):
        pixbuf = GdkPixbuf.Pixbuf().new_from_file_at_size(base_dir + "/images/zsh_previews/"+self.zsh_theme.get_active_text()+".jpg", image_width, image_height)
    image = Gtk.Image().new_from_pixbuf(pixbuf)
    image.set_margin_top(30)

    self.zsh_theme.connect("changed", self.update_image, image, "zsh", base_dir, image_width, image_height)
